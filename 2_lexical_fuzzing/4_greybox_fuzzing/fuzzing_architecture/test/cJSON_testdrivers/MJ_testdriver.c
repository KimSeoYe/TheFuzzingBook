...
set_configs (test_config_t * config)
{
    ...
    strcpy(config->runargs.binary_path, "../../lib/cJSON/testprograms/MJ_testCJSON") ;
...
main (int argc, char * argv[])
{
    ...
    for (int i = 0; i < loop_trials; i++) {
        execute_fuzzer(trials, RANDOM, "mj_random.csv") ;
    }
    clock_t rand_end = clock() ;

    clock_t mut_start = clock() ;
    for (int i = 0; i < loop_trials; i++) {
        execute_fuzzer(trials, MUTATION, "mj_mutation.csv") ;
    }
