...
set_configs (test_config_t * config)
{
    ...
    strcpy(config->runargs.binary_path, "../../lib/cJSON/testprograms/JY_cjson_test") ;
...
main (int argc, char * argv[])
{
    ...
    for (int i = 0; i < loop_trials; i++) {
        execute_fuzzer(trials, RANDOM, "jy_random.csv") ;
    }
    clock_t rand_end = clock() ;

    clock_t mut_start = clock() ;
    for (int i = 0; i < loop_trials; i++) {
        execute_fuzzer(trials, MUTATION, "jy_mutation.csv") ;
    }
