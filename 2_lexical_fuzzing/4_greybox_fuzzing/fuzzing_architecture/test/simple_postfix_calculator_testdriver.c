...
int 
calculator_oracle (int return_code, int trial)
{
    if (return_code != 0) {
        print_stderr(trial) ;
        return -1 ;   
...
{
    strcpy(config->runargs.binary_path, "../lib/simple_postfix_calculator/simple_postfix_calculator") ;
...
main (int argc, char * argv[])
{
    ...
    free(config.covargs.source_paths[0]) ;
